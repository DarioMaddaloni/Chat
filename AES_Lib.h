...
void decrypt(uint8_t buf[],int inlength, uint8_t roundKey[NR_ROUNDS+1][WORDS_IN_KEY][BYTES_IN_WORD]);
void decryptCBC(uint8_t buf[] ,int inlength, uint8_t roundKey[NR_ROUNDS+1][WORDS_IN_KEY][BYTES_IN_WORD], uint8_t iv[BLOCK_SIZE]);

uint8_t HexToNibble(char Hex);
void HexToBytes(char Hex[],int dimHex, uint8_t Byte[]);
